...
std::vector<CrashRecord> ProcessorUsingThreads::getCrashesInDateRange(const std::string& start_date, const std::string& end_date) {
    auto start = std::chrono::high_resolution_clock::now();
    ...
    for (size_t i = 0; i < records.size(); i++) {
        if (records[i].crash_date >= start_date && records[i].crash_date <= end_date) {
            #pragma omp critical
            filtered_crashes.push_back(records[i]);
        }
    }
...
std::vector<CrashRecord> ProcessorUsingThreads::getCrashesByInjuryCountRange(int min_injuries, int max_injuries) {
    auto start = std::chrono::high_resolution_clock::now();
    std::vector<CrashRecord> filtered_crashes;
...
std::vector<CrashRecord> ProcessorUsingThreads::getCrashesByLocationRange(float lat, float lon, float radius) {
    auto start = std::chrono::high_resolution_clock::now();
    std::vector<CrashRecord> filtered_crashes;
